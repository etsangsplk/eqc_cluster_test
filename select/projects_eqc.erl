...
-record(state, {org = undefined, ids = [], projects = []}).
...
create(Org, Id) ->
    ...
    eqc:format("Create project org_id=~p id=~p\n", [OrgId, Id]),
    #{id => Id, org_id => OrgId}.

create_next(State = #state{ids = Ids, projects = Projects}, User, [_Org, Id]) ->
    State#state{org = undefined, ids = Ids ++ [Id], projects = Projects ++ [User]}.
